...
def recommend_tracks(user_input_name, user_input_artist):
    ...
    if not seed_track_features:
        print(f"Não foi possível obter características para a música semente ({seed_track_info['name']} by {seed_track_info['artist']}).")
        return

    # Obtém as características de músicas recomendadas com base na música semente
    recommendations_same_artist = sp.recommendations(seed_tracks=[seed_track_info['id']], limit=5)['tracks']
    ...
    if len(recommendation_features_same_artist) >= 5:
        ...
        remaining_recommendations = sp.recommendations(seed_tracks=[seed_track_info['id']], limit=5 - len(recommendation_features_same_artist))['tracks']
